...
public class equipAddServlet extends ViewBaseServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try{
            ...
            String Datestr = request.getParameter("e_date");
            Date e_date = Date.valueOf(request.getParameter("e_date"));
            equipmentDAO.addEquipment(new Equipment(0,e_name,e_count,e_date));
            response.sendRedirect("equipment");
        }catch (Throwable e){
                response.setContentType("text/html;charset=utf-8");
                PrintWriter out=response.getWriter();
                out.print("<script language='javascript'>alert('参数不合法，请检查输入参数');window.location.href='equipment/add.html';</script>");
        }
