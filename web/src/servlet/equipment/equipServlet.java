...
public class equipServlet extends ViewBaseServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int equipCount = equipmentDAO.getEquipCount(keyword);
        int pagecount = (equipCount+7-1)/7;
        List<Equipment> list = new ArrayList<>();
        list = equipmentDAO.getEquipmentList(keyword,pageNo);
        session.setAttribute("pagecount",pagecount);
        session.setAttribute("keyword",keyword);
        session.setAttribute("pageNo",pageNo);
        session.setAttribute("equiplist",list);
        super.processTemplate("equipment",request,response);
    }
}
