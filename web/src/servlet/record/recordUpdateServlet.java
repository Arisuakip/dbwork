...
public class recordUpdateServlet extends ViewBaseServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try{
            ...
            //执行更新
            recordDAO.updateRecord(new Record(r_rid,r_eid,r_tid,r_date,r_remark,r_ename,r_tname));
            response.sendRedirect("record");
        }catch (Throwable e){
            response.setContentType("text/html;charset=utf-8");
            PrintWriter out=response.getWriter();
            out.print("<script language='javascript'>alert('参数不合法，请检查输入参数');window.location.href='record';</script>");
        }
