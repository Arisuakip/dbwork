...
public class memAddServlet extends ViewBaseServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try{
            ...
            String t_teachername = memberDAO.getMemberById(t_teacherid).getT_teachername();
            ...
            memberDAO.addMember(new Member(t_type,t_id,t_name,t_sex,t_phone,t_teacherid,t_level,t_teachername));
            response.sendRedirect("member");
        }catch (Throwable e){
            response.setContentType("text/html;charset=utf-8");
            PrintWriter out=response.getWriter();
            out.print("<script language='javascript'>alert('参数不合法，请检查输入参数');window.location.href='member/add.html';</script>");
        }
